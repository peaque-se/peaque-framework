...
function generateFrontendRouterJs(router: RouteNode, imports: Set<string>): string {
  const result: string[] = []
...
async function extractHeadStacks(router: RouteNode, basePath: string, pagesDir: string): Promise<Map<string, { headStack: string[], html: string }>> {
  ...
  function traverse(node: RouteNode, currentPath: string, accumulatedHeads: string[]) {
...
function generateBackendServerCode(apiRouter: RouteNode, headStacks: Map<string, { headStack: string[], html: string }>, pageRouter: RouteNode, basePath: string): string {
  ...
  function traverseApiRoutes(node: RouteNode, currentPath: string = "") {
  ...
  function generateApiRouteRegistrations(node: RouteNode, currentPath: string = "") {
    if (node.names?.handler) {
      const handlerFile = node.names.handler
      const relativePath = path.relative(basePath, handlerFile).replace(/\\/g, "/")
      const alias = relativePath.replace(/[^a-zA-Z0-9]/g, "_")
  ...
  function generatePageRouteRegistrations(node: RouteNode, currentPath: string = "", accumulatedHeads: string[] = []) {
    const heads = node.stacks?.heads ? [...accumulatedHeads, ...node.stacks.heads] : accumulatedHeads
...
export const buildForProduction = async (basePath: string) => {
  ...
  const frontend = buildRouter(basePath + "/src/pages", pageRouterConfig)
  const frontendImports = componentify(frontend, basePath + "/src/pages")
  const frontendJs = generateFrontendRouterJs(frontend, frontendImports)
  ...
  const backend = buildRouter(basePath + "/src/api", apiRouterConfig)
